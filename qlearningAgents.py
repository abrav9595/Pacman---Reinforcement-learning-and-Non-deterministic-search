...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        """
        return sum([self.weights[feature]*value for feature, value in self.featExtractor.getFeatures(state,action).iteritems()])

    def update(self, state, action, nextState, reward):
        """
           Should update your weights based on transition
        """
        difference = self.discount*self.computeValueFromQValues(nextState) - self.getQValue(state,action)
        for feature, value in self.featExtractor.getFeatures(state,action).iteritems():
          self.weights[feature] += self.alpha * (reward + difference) * value
    ...
    def final(self, state):
        ...
        if self.episodesSoFar == self.numTraining:
            ...
            print "Weights obtained = "+str(self.weights)
            pass
